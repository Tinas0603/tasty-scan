                ...
                <TableBody>
                  {table.getRowModel().rows?.length ? (
                    table.getRowModel().rows.map((row) => {
                      const isSelectable = row.original.status !== 'Unavailable' && row.original.status !== 'Hidden';

                      return (
                        <TableRow
                          key={row.id}
                          data-state={row.getIsSelected() && 'selected'}
                          onClick={() => isSelectable && choose(row.original)}
                          className={`cursor-pointer ${!isSelectable ? 'opacity-50 pointer-events-none' : ''}`}
                        >
